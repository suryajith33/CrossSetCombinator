...
export const CombinationType: React.FunctionComponent = () => {
	...
	const [combinations, setCombinations] = useState<CombinationModel>(combination);
	...
	const handleOptionValue = (newValue: string, type_index: number, opt_index: number) => {
	...
	return (
		<>
			<div>
				<button onClick={() => console.log(combinations)}>CLick</button>
				{type?.map((item, type_index) => (
					<div key={type_index}>
						...
						<button
							onClick={() => {
								const updatedType = type.filter((_, index) => type_index !== index);
								setType(updatedType);
							}}
						>
							Delete
								...
								<>
									...
									<button
										onClick={() => {
											...
											const updatedOptions = variantType.options.filter((_, idx) => idx !== opt_index);
